  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-gold text-navy rounded-lg hover:bg-gold/90 transition-all text-sm font-semibold mt-auto"
      >
        <Sparkles className="w-4 h-4" /> Request Service
      </button>
    </div>
  );
}
    ...
    <div className="py-20 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {services.map(s => (
            <ServiceCard key={s.id} service={s} onRequest={(name) => openHireMe(name)} />
          ))}
        </div>
      </div>
    </div>
